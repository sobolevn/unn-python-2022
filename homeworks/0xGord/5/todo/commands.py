...
class DoneCommand(BaseCommand):
    ...
    def perform(self, store):
        ...
        while True:
            try:
                selected = self._select_item(store)
            except ValueError:
                print('Bad input, try again.')
            except IndexError:
                ...
                break
        store.items[selected].done = True
        print(f"Entry {str(store.items[selected])} added")
    ...
    def perform(self, store):
        ...
        while True:
            try:
                ...
                break
        store.items[selected].done = False
        print(f"Entry {str(store.items[selected])} is undone")
