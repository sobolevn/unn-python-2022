class BaseItem(object):
    ...
    #def __str__(self):
    #    return self.__class__.__name__[:-4]   #Return class name without 'Item' 

    def __str__(self):
        return '{0} {1}'.format(
            '+' if self.done else '-',       #Статус готовности
            self.__class__.__name__[:-4],    #Название типа задачи (без'Item') 
          )  
    @classmethod
    def construct(cls):
