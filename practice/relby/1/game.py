...
def field_from_flat_field(flat_field: list[int], size: int) -> Field:
    ...
    :param flat_field: Flat field
    :param size: Size of each row
    :return: Field object
    """
    return [flat_field[i*size:i*size + size] for i in range(size)]
...
# Consts
FIELD_SIZE = 4
EMPTY_CELL = FIELD_SIZE**2 - 1
WINNING_FIELD = field_from_flat_field(list(range(FIELD_SIZE**2)), FIELD_SIZE)

PADDING = len(str(EMPTY_CELL)) + 1
# This should grow depending on FIELD_SIZE
SHUFFLE_ITERATIONS = FIELD_SIZE*1000
...
def print_field(field: Field) -> None:
    ...
    for row in field:
        for cell in row:
            cell_to_render = EMPTY_MARK if cell == EMPTY_CELL else str(cell + 1)
            print(cell_to_render.rjust(PADDING), end='')
        print()

def reprint_field(field: Field) -> None:
    print('\x1b[{0}A'.format(FIELD_SIZE), end='')
    print('\x1b[{0}D'.format(FIELD_SIZE*PADDING), end='')
    print_field(field)
...
def perform_move(field: Field, key: str) -> Field | None:
    ...
    if y2 < 0 or y2 >= FIELD_SIZE or x2 < 0 or x2 >= FIELD_SIZE:
        return None
